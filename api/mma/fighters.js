module.exports = async function handler(req, res) {
  ...
  try {
    const response = await fetch("https://api.sportsdata.io/v3/mma/scores/json/FightersBasic", {
      headers: { "Ocp-Apim-Subscription-Key": sportsDataKey }
    });
    const data = await response.json();
    res.setHeader("Cache-Control", "no-store");
    res.status(200).json({ fetchedAt: new Date().toISOString(), fighters: Array.isArray(data) ? data : [] });
  } catch (err) {
